...
// TODO: The business logic in these middleware can likely be pulled out to a shared location.
const { findFavicon } = require('./middleware/favicon');
const makeWebpackConfig = require('./middleware/webpack/config');
...
class VapidBuilder extends Vapid {
  ...
  async build(dest) {
    ...
    for (const asset of assets) {
      if (!isAssetPath(asset)) { continue; }
      try { assertPublicPath(asset); } catch (err) { continue; }
      const out = path.join(dest, path.relative(this.paths.www, asset));
      Utils.mkdirp(path.dirname(out));
      fs.copyFileSync(asset, out);
    }

    // Copy discovered favicon over.
    const faviconPath = findFavicon([this.paths.www, getDashboardPaths().assets]);
    if (faviconPath) {
    ...
    for (const tmpl of templates) {
      ...
      if (isTemplatePartial(tmpl)) {
        ...
        // If this Records collection folder stomps on the namespace of a dedicated page,
        // move that page to `recordName/index.html`. If the record name is 'index' then we
        // need to duplicate the `index.html` file for folder indexing to work as expected.
        const oldFile = path.join(dest, `${name}.html`);
        if (fs.existsSync(oldFile)) {
          const method = name === 'index' ? fs.copyFileSync : fs.renameSync;
          method(oldFile, path.join(dest, name, 'index.html'));
        }

      // If this isn't a partial, it must be a page. Render it! If a Records directory
      // of the same name already exists, create this page at `recordName/index.html`.
      // If the record name is 'index' then we need to duplicate the `index.html` file for
      // folder indexing to work as expected.
      } else {
        const out = Utils.isDirectory(path.join(dest, parsed.name)) ? path.join(parsed.name, 'index.html') : parsed.base;
        await this.renderUrl(`/${parsed.name}`, path.join(dest, out));
        if (parsed.name === 'index' && out !== 'index.html') {
          fs.copyFileSync(path.join(dest, out), path.join(dest, 'index.html'));
          Logger.extra(['Created: /index.html']);
        }
        Logger.extra([`Created: /${out}`]);
      }
    }
