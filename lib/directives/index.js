...
function find(params = {}, meta = {}) {
  ...
  if (availableDirectives[name]) {
    return new availableDirectives[name](params, meta);
  }
  ...
  return new availableDirectives.text(params, meta);
}

function helper(value, attrs, meta) {
  const out = find(attrs, meta).render(value);
  return () => (typeof out === 'string' ? new Handlebars.SafeString(out) : out);
}

function get(name) {
  return availableDirectives[name];
}
