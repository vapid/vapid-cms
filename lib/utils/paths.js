...
exports.getRecordFromPath = async function getRecordFromPath(permalink, db) {
  ...
  if (permalink.includes('/')) {
    ...
    const template = await Template.findOne({ where: { type, name: collection } });
    if (!template) { return null; }

    // Try to get the plain old slug value if it exists.
    template.records = await template.getRecords(slug ? { where: { slug }, include: ['template'] } : { include: ['template'] });
  ...
  return (await template.getRecords({ order: [['id', 'ASC']], include: ['template'] }) || [])[0] || null;
