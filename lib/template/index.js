...
function parseLeaf(str) {
  ...
  if (!key) { key = context; context = null; }
  // If there is a key and context, strip the leading `.` from the key.
  ...
  return { key, context, path, params, remainder };
}
  ...
  branch.forEach((leaf) => {
    switch (leaf[0]) {
      ...
      case '#': {
        ...
        if (!!~Goatee.CONDITIONALS.indexOf(parsedLeaf.key)) {
...
function _replacePartials(html, partials) {
  ...
  if (!Utils.isEmpty(partials)) {
    for (let i=0; i<PARTIALS_DEPTH; i++) {
