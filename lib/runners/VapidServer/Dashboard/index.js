...
router.post('records#create', '/:type/:name/records', findSection, async (ctx) => {
  ...
  try {
    ...
    const name = template.type === 'page' ? await record.safeSlug() : template.name;
...
async function _newRecordAction(ctx, errors = {}) {
  ...
  const { template } = ctx.state;
  const title = `New ${record.nameSingular || template.labelSingular} ${template.type === 'page' ? 'Page' : ''}`;
  await ctx.render('records/new', title, {
...
async function _editRecordAction(ctx, record = {}, errors = []) {
  const { template } = ctx.state;
  await ctx.render('records/edit', record.nameSingular || template.labelSingular, {
    record,
