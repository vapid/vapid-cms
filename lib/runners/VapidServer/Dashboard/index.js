...
const { getRecordFromPath } = require('../../../utils/paths');
...
router.use(async (ctx, next) => {
  if (ctx.isAuthenticated()) {
    ...
    ctx.state.pages = pages.map(p => p.getMetadata());
    ...
    for (const collection of ctx.state.collections) {
      collection.hasPage = !!(await Template.pageFor(collection.name));
    }
...
router.get('deploy', '/deploy', async (ctx) => {
  ...
  const builder = new VapidBuilder(sitePaths.root, true);
  try {
...
router.get('sections#pages', '/pages', async (ctx) => {
  ...
  const templates = [];
  for (const t of await Template.scope('pages').findAll()) {
    t.collection = await Template.collectionFor(t.name);
    templates.push(t);
  }
...
router.get('sections#index', '/:type/:name', findSection, async (ctx, errors = []) => {
  ...
  // If this is the type of template that contain multiple records, render the records list page.
    ...
    return ctx.render('records/index', ctx.state.template.label, {
      collection: await Template.collectionFor(template.name),
      page: await Template.pageFor(template.name),
      tableAction,
      csrf: ctx.csrf,
      Form,
...
async function findPage(ctx, next) {
  ...
  const record = await getRecordFromPath(permalink, Template.sequelize);
  if (record) {
