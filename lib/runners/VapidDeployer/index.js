...
class VapidDeployer extends Vapid {
  ...
  async deploy() {
    ...
    this.credentials = this.getCredentials(this.apiHostname);
    ...
    siteChecksums = this.getChecksums();

    // Upload
    Logger.info('Preparing upload');
    const siteAndUploadsChecksums = this.getChecksums();
    presignedPosts = await this.getPresignedPosts(siteAndUploadsChecksums);
  ...
  async ensureSite() {
    ...
    // Create a new site
    const deployContent = answers.deployContent;
    ...
    // Update package.json
    const pjson = require(this.paths.pjson); // eslint-disable-line global-require, import/no-dynamic-require
