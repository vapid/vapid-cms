...
class TemplateCompiler {
  ...
  walk(data, node, aliases = {}) {
    ...
    switch (node.type) {
      ...
      case 'SubExpression': {
        ...
        if (node.params && node.params.length) {
          for (const param of node.params) {
            this.walk(data, param, param, aliases);
          }
      ...
      case 'BlockStatement': {
        // All Block statements are helpers. Crawl all its params as potential data values in scope.
...
function addToTree(data, leaf, path, aliases) {
  ...
  const isPrivateKey = aliases[leaf.key] ? aliases[leaf.key].isPrivate : false;
